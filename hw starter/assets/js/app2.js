...
function xScale(Data, chosenXAxis) {
  ...
  function updateToolTip(chosenXAxis, circlesGroup) {
    ...
    var toolTip = d3.tip()
      ...
      .offset([80, -60])
      .html(function(d) {
        return (`${d.state}<br>${label} ${d[chosenXAxis]}<br>Age:${d.age} `);
      });
...
d3.csv("assets/data/data.csv").then(function(DS, err) {
  if (err) throw err;
   
  // parse data - set values to numerical data types
  DS.forEach(function(data) {
    data.poverty = +data.poverty; //x-axis used to be in activity (hair_length)
    data.age = +data.age;    //y-axis used to be in activitty (num_hits)
    data.abbr = data.abbr;
  ...
  // Create y scale function to fit  pixels in the chart 
  var yLinearScale = d3.scaleLinear()
    .domain([d3.min(DS, d => d.age -5), d3.max(DS, d => d.age +5)])
    .range([height, 0]);
  ...
  var circlesGroupAll = chartGroup.selectAll("circlesGroup").data(DS).enter();
